...
def main():
    ...
    """
    Application.set_input_camera_video(frames=10000)

    Application.delete_folder_appl_out()
    Benchmarking.delete_folder_benchmark_out()

    Application.do_get_video_capture_job(port_output_name='RAW')

    Application.do_pyramid_level_down_job(port_input_name='RAW', port_input_lvl=CONFIG.PYRAMID_LEVEL.LEVEL_0,
                                          port_output_name='RAW', number_of_lvl=2, verbose=False, is_rgb=True)

    level = CONFIG.PYRAMID_LEVEL.LEVEL_0

    Application.do_grayscale_transform_job(port_input_name='RAW', port_output_name='RAW_GRAY', level=level)

    Application.do_median_pixel_image_job(port_input_name='RAW_GRAY',
                                          port_output_name='MEDIAN_RAW_GRAY', level=level)

    Application.do_canny_otsu_median_sigma_job(port_input_name='RAW_GRAY', edge_detector=CONFIG.FILTERS.ORHEI_3x3,
                                               port_output_name='CANNY_OTSU_MEDIAN_SIGMA_RAW_GRAY', level=level,
                                               do_blur=True, kernel_blur_size=9, sigma=1.4)
