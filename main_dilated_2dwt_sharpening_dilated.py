...
def main_paper():
    ...
    #
    # um_2dwt = Application.do_um_2dwt_fusion(port_input_name=input, is_rgb=is_rgb, octaves=5, s=1.3, k=math.sqrt(2), m=1, wavelet='db4', port_output_name='UM_2DWT')
    # eval_list.append(um_2dwt)
    # Application.do_matrix_difference_job(port_input_name_1=um_2dwt, port_input_name_2=input, normalize_image=True, save_cmap=True)
    # #
    um_d_2dwt = Application.do_unsharp_filter_dilated_2dwt_job(port_input_name=input, is_rgb=is_rgb, kernel=CONFIG.FILTERS_SECOND_ORDER.LAPLACE_1, wave_lenght='haar',
                                                               strenght=0.9, levels_fusion=2, fusion_rule='average', port_output_name='UM_PROPOSED')
    eval_list.append(um_d_2dwt)
    Application.do_matrix_difference_job(port_input_name_1=um_d_2dwt, port_input_name_2=input, normalize_image=True, save_cmap=True, is_rgb=is_rgb)
    ...
    list_to_plot = [
        ('UM_V1_GRAY',
         [set for set in eval_list if ('UM_' in set) or set == 'GRAY_RAW_L0'],
