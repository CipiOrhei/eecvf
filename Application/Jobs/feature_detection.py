...
def main_func_kaze(param_list: list = None) -> bool:
    """
    Main function for A-KAZE calculation job.
    :param param_list: Param needed to respect the following list:
                       [input_port_name, input_port_wave, number_of_features, number_of_octaves, contrast_threshold, edge_threshold,
    ...
    PORT_IN_N_OCTAVES = 4
    # noinspection PyPep8Naming
    PORT_IN_N_OCTAVES_LAYERS = 5
    # noinspection PyPep8Naming
    PORT_IN_DIFFUSIVITY = 6
    # noinspection PyPep8Naming
    PORT_IN_MASK = 7
    # noinspection PyPep8Naming
    PORT_IN_SAVE_NPY = 8
    ...
    PORT_IN_SAVE_TXT = 9
    # noinspection PyPep8Naming
    PORT_OUT_KEYPOINTS = 10
    # noinspection PyPep8Naming
    PORT_OUT_DESCRIPTORS = 11
    # noinspection PyPep8Naming
    PORT_OUT_IMG = 12

    # verify that the number of parameters are OK.
    if len(param_list) != 13:
        log_error_to_console("KAZE JOB MAIN FUNCTION PARAM NOK", str(len(param_list)))
        return False
    else:
        # get needed ports
        p_in = get_port_from_wave(name=param_list[PORT_IN_POS], wave_offset=param_list[PORT_IN_WAVE])
        ...
        if p_in.is_valid() is True:
            try:
                a_kaze_obj = cv2.KAZE_create(threshold=param_list[PORT_IN_THR],
                                             nOctaves=param_list[PORT_IN_N_OCTAVES], nOctaveLayers=param_list[PORT_IN_N_OCTAVES_LAYERS],
                                             diffusivity=param_list[PORT_IN_DIFFUSIVITY])
...
def do_kaze_job(port_input_name: str, number_features: int = 1024,
                threshold: float = 0.001, nr_octaves: int = 4, nr_octave_layers: int = 4, diffusivity: int = cv2.KAZE_DIFF_PM_G2,
    ...
    :param diffusivity: Diffusivity type.
           cv2.KAZE_DIFF_PM_G1, g1 = exp(-|dL|^2/k^2)
           cv2.KAZE_DIFF_PM_G2,  g2 = 1 / (1 + dL^2 / k^2)
           cv2.KAZE_DIFF_CHARBONNIER, gc = 1 / sqrt(1 + dL^2 / k^2)
           cv2.KAZE_DIFF_WEICKERT, g3 = 1 or  1 - exp(-3.315/(|dL|/k)^2)
    :param mask_port_name: Masks for each input image specifying where to look for keypoints
    ...
    if port_kp_output is None:
        port_kp_output = 'KAZE_KP_THR_{thr}_NO_{no}_NOL_{nol}_D_{d}_{INPUT}'.format(thr=threshold.__str__().replace('.', '_'), d=diffusivity.__str__(),
    ...
    if port_des_output is None:
        port_des_output = 'KAZE_DES_THR_{thr}_NO_{no}_NOL_{nol}_D_{d}_{INPUT}'.format(thr=threshold.__str__().replace('.', '_'), d=diffusivity.__str__(),
                                                                                      no=nr_octaves.__str__(), nol=nr_octave_layers.__str__().replace('.', '_'),
                                                                                      INPUT=port_input_name)
        if mask_port_name is not None:
            port_des_output += '_MASKED_BY_' + mask_port_name
    ...
    if port_img_output is None:
        port_img_output = 'KAZE_IMG_THR_{thr}_NO_{no}_NOL_{nol}_D_{d}_{INPUT}'.format(thr=threshold.__str__().replace('.', '_'), d=diffusivity.__str__(),
                                                                                      no=nr_octaves.__str__(), nol=nr_octave_layers.__str__().replace('.', '_'),
                                                                                      INPUT=port_input_name)
        if mask_port_name is not None:
            port_img_output += '_MASKED_BY_' + mask_port_name
    ...
    job_name = job_name_create(action='KAZE', input_list=input_port_list, wave_offset=[wave_offset], level=level)
...
def do_a_kaze_job(port_input_name: str, number_features: int = 4*1024,
    ...
    :param nr_octave_layers: Default number of sublevels per scale level
    :param diffusivity: Diffusivity type.
           cv2.KAZE_DIFF_PM_G1 = 0, g1 = exp(-|dL|^2/k^2)
           cv2.KAZE_DIFF_PM_G2 = 1,  g2 = 1 / (1 + dL^2 / k^2)
           cv2.KAZE_DIFF_CHARBONNIER = 3 , gc = 1 / sqrt(1 + dL^2 / k^2)
           cv2.KAZE_DIFF_WEICKERT = 2, g3 = 1 or  1 - exp(-3.315/(|dL|/k)^2)
    :param mask_port_name: Masks for each input image specifying where to look for keypoints
    :param save_to_text: if we want to save BOW clusters to txt files
    :param save_to_npy: if we want to save BOW clusters to npy files
    :param port_kp_output: Output port name for KeyPoints list
