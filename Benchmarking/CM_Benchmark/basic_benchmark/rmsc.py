...
def rmsc(img):
    ...
    mean_intesity = img.mean()

    tmp = (img - mean_intesity)
    tmp = tmp ** 2
    sum_tmp = tmp.sum()

    rmsc = math.sqrt(1/(M*N)*sum_tmp)

    return rmsc


# noinspection PyPep8Naming
def run_RMSC_benchmark(input_location: str, raw_image: str, jobs_set: list):
    """
    xxx
    ...
    run_CM_benchmark_RMSC()


def run_CM_benchmark_RMSC():
    """
    :return:
    """
    log_setup_info_to_console("BENCHMARKING CM RMSC")
    idx = 0
    ...
    for set in config_main.BENCHMARK_SETS:
        ...
        if True:
            ...
            results_path = os.path.join(os.getcwd(), config_main.BENCHMARK_RESULTS, "RMSC")

            if not os.path.exists(results_path):
                os.makedirs(results_path)

            csv = open(os.path.join(results_path, set + '.log'), "w+")
            csv.write('Per image (#, RMSC' + ':\n')
            ...
            for file in config_main.BENCHMARK_SAMPLE_NAMES:
                ...
                try:
                    ...
                    log_error_to_console("BENCHMARK CM RMSC: {file}".format(file=file), ex.__str__())

            log_benchmark_info_to_console('RMSC: {set:<10s} {cnt:<10.6f}\n'.format(set=set, cnt=avg / count))
            csv.write('RMSC: {set:<10s} {cnt:<10.6f}\n'.format(set=set, cnt=avg / count))
