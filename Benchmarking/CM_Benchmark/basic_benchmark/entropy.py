...
def run_Entropy_benchmark(input_location: str, raw_image: str, jobs_set: list):
    """
    xxx
    :param input_location: location of algorithm images
    :param raw_image: location of raw images dataset
    ...
    run_CM_benchmark_Entropy()


def run_CM_benchmark_Entropy():
    """
    :return:
    """
    log_setup_info_to_console("BENCHMARKING CM Entropy")
    idx = 0
    ...
    for set in config_main.BENCHMARK_SETS:
        ...
        if True:
            ...
            results_path = os.path.join(os.getcwd(), config_main.BENCHMARK_RESULTS, "Entropy")

            if not os.path.exists(results_path):
                os.makedirs(results_path)

            csv = open(os.path.join(results_path, set + '.log'), "w+")
            csv.write('Per image (#, Entropy' + ':\n')
            ...
            for file in config_main.BENCHMARK_SAMPLE_NAMES:
                ...
                try:
                    val = skimage.measure.shannon_entropy(img_al)
                    avg += val
                    count += 1
                    csv.write('{:<10s} {:<10.6f}\n'.format(file, val))
                    # log_benchmark_info_to_console('{:<10s} {:<10.6f}\n'.format(file, val))
                except Exception as ex:
                    log_error_to_console("BENCHMARK CM Entropy: {file}".format(file=file), ex.__str__())

            log_benchmark_info_to_console('Entropy: {set:<10s} {cnt:<10.6f}\n'.format(set=set, cnt=avg / count))
            csv.write('Entropy: {set:<10s} {cnt:<10.6f}\n'.format(set=set, cnt=avg / count))
