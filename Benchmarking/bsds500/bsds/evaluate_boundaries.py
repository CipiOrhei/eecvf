...
try:
    ...
    log_setup_info_to_console("CORRESPONDING_PIXEL ALGORITHM DOESN'T WORK ON WINDOWS")
...
# noinspection PyPep8
def pr_evaluation(thresholds, thinning, sample_names, set_name, load_gt_boundaries, load_pred, max_distance_px,
                  progress=None):
    """
    ...
    for sample_index, sample_name in enumerate(progress(sample_names)):
        ...
        # Load them
        pred = load_pred(sample_name, set_name)
        gt_b = load_gt_boundaries(sample_name)

        value_px = round(max_distance_px / int(np.sqrt(len(pred) ** 2 + len(pred[0]) ** 2)), 5)

        # Evaluate predictions
        count_r, sum_r, count_p, sum_p, used_thresholds = \
            evaluate_boundaries(predicted_boundaries=pred, gt_boundaries=gt_b,
                                max_dist=value_px,
                                thresholds=thresholds, apply_thinning=thinning)
